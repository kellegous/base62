...
func TestEncoding(t *testing.T) {
	for _, trial := range trials {
		var b bytes.Buffer
		e := NewEncoder(&b)
		if _, err := io.Copy(e, bytes.NewBuffer(trial.decoded)); err != nil {
			t.Fatalf("%v\n", err)
		}
		e.Close()

		if trial.encoded != b.String() {
...
func TestDecoding(t *testing.T) {
	for _, trial := range trials {
		...
		if !bytes.Equal(trial.decoded, b.Bytes()) {
			t.Fatalf("expected %v got %v (for %s)", trial.decoded, b.Bytes(), trial.encoded)
		}
	}
}
